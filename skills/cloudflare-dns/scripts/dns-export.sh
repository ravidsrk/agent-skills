...
settings = json.load(open(f"{raw}/settings.json")).get('result', []) or []
rulesets = json.load(open(f"{raw}/rulesets.json")).get('result', []) or []
dnssec   = json.load(open(f"{raw}/dnssec.json")).get('result') or {}
...
def short_name(name):
    return '@' if name == domain else (
        name[:-(len(domain)+1)] if name.endswith('.' + domain) else name
    )
...
if fmt == 'yaml':
    ...
    open(output, 'w').write('\n'.join(out) + '\n')
    print(f"  (OK) Exported {len(records)} records, {len(settings_d)} settings, {len(rulesets)} rulesets")
    print(f"     to: {output}")
...
# ---------------------------------------------------------------------------
    ...
    for r in records:
        ...
        if rtype == 'MX':
            ...
            lines.append(f"{short:<30} {r['ttl']:<6} IN {rtype} {r['content']}")
    open(output, 'w').write('\n'.join(lines) + '\n')
    print(f"  (OK) Exported BIND zonefile -> {output}")

# ---------------------------------------------------------------------------
elif fmt == 'terraform':
    # Cloudflare Terraform provider v4+: cloudflare_dns_record + content (was cloudflare_record + value in v3).
    safe = domain.replace('.', '_').replace('-', '_')
    lines = [
        f"# Cloudflare zone — {domain} (auto-generated by the cloudflare-dns skill)",
        f"# To import: terraform import cloudflare_zone.{safe} {zone['id']}",
        "# Requires the cloudflare Terraform provider v4 or newer.",
        "",
    ...
    for r in records:
        ...
        slug_base = rname.replace('@','apex').replace('*','wildcard').replace('.','_').replace('-','_')
        slug = f"{r['type'].lower()}_{slug_base}"
        lines.append(f'resource "cloudflare_dns_record" "{slug}" {{')
        lines.append(f'  zone_id = cloudflare_zone.{safe}.id')
        lines.append(f'  name    = "{rname}"')
        lines.append(f'  type    = "{r["type"]}"')
        if r['type'] == 'MX':
            lines.append(f'  priority = {r.get("priority",10)}')
        lines.append(f'  content = {json.dumps(r["content"])}')
        ...
        lines.append('')
    open(output, 'w').write('\n'.join(lines) + '\n')
    print(f"  (OK) Exported Terraform (provider v4) -> {output}")
